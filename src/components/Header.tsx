...
export default function Header() {
  ...
  const getCurrentThemeForDisplay = () => {
    if (theme === "auto") {
  ...
  const themeIcon = getCurrentThemeForDisplay();

  return (
    <div
      className={`fixed left-4 right-4 lg:right-0 lg:left-0 z-20 px-4 py-1 lg:p-4 items-center lg:rounded-none rounded-3xl lg:rounded-bl-2xl shadow-md lg:ml-auto w-auto lg:w-max backdrop-blur-2xl lg:block head top-4 lg:top-0 transition-all duration-500
      ...
      <nav className="flex items-center justify-between h-18 lg:h-7 xl:h-max md:flex lg:gap-4 xl:gap-10 font-medium">
        ...
        {[
          ...
          <Link
            key={idx}
            href={href}
            className={`group hidden lg:flex flex-row items-center text-sm px-3 py-2 rounded-2xl transition-all duration-300 hover:bg-blue-500 hover:text-white
              ${themeIcon === "light" ? "text-blue-500" : "text-blue-500"}`}
          >
            <span className="flex items-center scale-80 xl:scale-100 gap-2">
              {svg}
        ...
        <span
          className="menu relative xl:scale-120 scale-120 lg:scale-90 cursor-pointer block"
        ...
        >
          ...
          {menuOpen && (
            <div
              className={`absolute right-0 top-15 w-44 rounded-xl shadow-lg ring-1 ring-black/10 dark:ring-white/10 p-3 transition-all duration-300 z-50
                ${themeIcon === "light" ? "bg-white" : "bg-gray-800"}`}
            >
              {[
                {
                  mode: "light" as const,
